...
func TestStatus(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			req, err := http.NewRequest(http.MethodGet, modelRunner.URL(inference.ModelsPrefix), http.NoBody)
			...
			if test.doResponse != nil && test.doResponse.StatusCode == http.StatusOK {
				req, err = http.NewRequest(http.MethodGet, modelRunner.URL(inference.InferencePrefix+"/status"), http.NoBody)
				require.NoError(t, err)
				req.Header.Set("User-Agent", "docker-model-cli/"+desktop.Version)
