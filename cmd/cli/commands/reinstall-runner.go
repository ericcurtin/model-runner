...
func newReinstallRunner() *cobra.Command {
	...
	c := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Remove any existing model runner containers (uninstall step).
			// We don't remove images or volumes to preserve data.
			if err := standalone.PruneControllerContainers(cmd.Context(), dockerClient, false, cmd); err != nil {
				return fmt.Errorf("unable to remove existing model runner container(s): %w", err)
			}
